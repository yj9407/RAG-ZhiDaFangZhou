...
graph_builder.add_node("verify_result", verify_result)  # ⭐ 新增：结果校验节点
...
# ⭐ execute_sql → verify_result（原直连 END，改为先校验）
graph_builder.add_edge("execute_sql", "verify_result")

# ⭐ verify_result 条件路由：通过→结束，不通过且未超重试次数→重新修正
