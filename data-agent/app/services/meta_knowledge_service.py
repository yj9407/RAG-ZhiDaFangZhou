    ...
    async def _save_tables_to_meta_db(self, meta_config: MetaConfig)-> list[ColumnInfo]:
        ...
        for table in meta_config.tables:
            ...
            for column in table.columns:
                ...
                # 将 Decimal 类型转为 float，确保 JSON 序列化兼容
                column_values = [float(v) if isinstance(v, Decimal) else v for v in column_values]
                # 构造ColumnInfo实例
    ...
    async def _save_column_info_to_qdrant(self, column_infos: list[ColumnInfo]):
        ...
        for column_info in column_infos:
            ...
            for alia in column_info.alias:
                points.append(
                    {"id": str(uuid.uuid4()), "embedding_text": alia, "payload": column_info}
                )
        # 向量列表
    ...
    async def _save_metric_info_to_qdrant(self, metric_infos: list[MetricInfo]):
        ...
        for metric_info in metric_infos:
            ...
            for alia in metric_info.alias:
                points.append(
                    {"id": str(uuid.uuid4()), "embedding_text": alia, "payload": metric_info}
                )
